...
Data is ingested from transactional SQL databases and static reference files (CSV seeds) representing core eCommerce entities:

- **Orders** — Transaction history with order details, status, and timestamps
- **Customers** — Customer profiles with demographics, contact info, and segments
- **Products** — Product catalog with pricing, categories, and inventory levels
- **Events** — Customer behavioral data (page views, clicks, conversions)
- **Payments** — Payment methods, transactions, and reconciliation data

### Source Systems Integration

| Source | Type | Frequency | Format |
|--------|------|-----------|--------|
| Transactional DB | SQL | Daily | Direct query |
| CSV Seeds | Static Reference | Monthly | Upload |
| API Events | Streaming | Real-time | JSON |
| Legacy Systems | Batch | Weekly | Parquet |
